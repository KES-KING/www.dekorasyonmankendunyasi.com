<footer class="site-footer">
    <div class="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 py-14 sm:px-6 lg:grid-cols-4 lg:px-8">
        <div class="lg:col-span-1">
            ...
            <p class="mt-4 text-xs uppercase tracking-[0.2em] text-yellow-600 font-bold"><?= e($settings['brand_name'] ?? 'Dekorasyon Manken Dünyası'); ?></p>
            <p class="mt-2 text-sm text-zinc-500"><?= e($settings['footer_tagline'] ?? 'Sizin İçin Tasarlandı'); ?></p>
        </div>
        ...
        <div>
            <h3 class="footer-title text-xs uppercase tracking-[0.24em] font-semibold"><?= e($settings['footer_manifesto_title'] ?? 'Hakkımızda'); ?></h3>
            <p class="mt-4 text-sm leading-6 text-zinc-500">
                <?= e($settings['footer_manifesto_text'] ?? 'Mekanlarınıza değer katmak için en özel manken ve dekorasyon ürünlerini sunuyoruz.'); ?>
            </p>
        </div>

        <div>
            <h3 class="footer-title text-xs uppercase tracking-[0.24em] font-semibold">Tüm İletişim</h3>
            <ul class="mt-4 space-y-2 text-sm">
    ...
    <div class="border-t border-black/5">
        <div class="mx-auto flex max-w-7xl flex-col items-start justify-between gap-2 px-4 py-5 text-xs text-zinc-400 sm:flex-row sm:items-center sm:px-6 lg:px-8">
            <p>&copy; <?= e((string) $currentYear); ?> <?= e($settings['brand_name'] ?? 'Dekorasyon Manken Dünyası'); ?>. Tüm hakları saklıdır.</p>
            <p><?= e(currentLocale()); ?> | <?= e($settings['footer_tagline'] ?? 'Sizin İçin Tasarlandı'); ?></p>
        </div>
    </div>
</footer>
